...
public static class CovenProgress
{
    ...
    public static bool IsContinentComplete(GameState state, ImmutableArray<WorldLocationDef> locations, string continent)
    {
        ...
        if (!covens.All(l => state.FindCoven(l.Id)?.Converted == true)) return false;

        var rival = RivalCultData.Rivals.FirstOrDefault(r => r.PreferredTerritoryId == continent);
        if (rival == null) return true;
        var rivalState = state.RivalCultsOrInit.GetRival(rival.Id);
        return rivalState?.Defeated == true;
    }
